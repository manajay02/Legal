...
class NotesUpdateRequest(BaseModel):
    """Request to update user notes for a document."""
    notes: Dict[str, str]  # section_id -> note text


@router.get("/documents/{doc_id}/notes")
async def get_document_notes(doc_id: str) -> Dict[str, Any]:
    """
    Get user notes for a document.
    Returns a dictionary of section_id -> note text.
    """
    db = get_db()
    doc_data = db.get(doc_id)
    if not doc_data:
    ...
    return {
        "document_id": doc_id,
        "notes": doc_data.get("user_notes", {})
    }


@router.put("/documents/{doc_id}/notes")
async def update_document_notes(doc_id: str, request: NotesUpdateRequest) -> Dict[str, Any]:
    """
    Update user notes for a document.
    Accepts a dictionary of section_id -> note text.
    """
    db = get_db()
    doc_data = db.get(doc_id)
    if not doc_data:
    ...
    db.update(doc_id, {"user_notes": request.notes})
...
@router.put("/documents/{doc_id}/notes/{section_id}")
async def update_section_note(doc_id: str, section_id: str, note: str = "") -> Dict[str, Any]:
    """
    Update a single section's note.
    """
    db = get_db()
    doc_data = db.get(doc_id)
    if not doc_data:
    ...
    # Get existing notes or create new dict
    notes = doc_data.get("user_notes", {})
    if note.strip():
        notes[section_id] = note
    elif section_id in notes:
        del notes[section_id]
    
    # Update the notes field
    db.update(doc_id, {"user_notes": notes})
