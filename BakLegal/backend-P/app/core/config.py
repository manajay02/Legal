...
class Settings(BaseSettings):
    ...
    UPLOADS_DIR: Path = Field(
        default_factory=lambda: Path(__file__).parent.parent.parent / "data" / "uploads",
    ...
    TESSERACT_CMD: Optional[str] = Field(
        default=None,
        validation_alias=AliasChoices("TESSERACT_CMD", "TESSERACT_PATH"),
        description="Path to tesseract executable. If None, will use system PATH."
    )
    TESSERACT_LANG: str = Field(
        default="eng+sin",
        validation_alias=AliasChoices("TESSERACT_LANG", "TESSERACT_LANGS"),
    ...
    def coerce_debug_bool(cls, v):
        """Coerce DEBUG from common string forms.

        Some environments set DEBUG to log-level strings (e.g., "WARN"),
        which would otherwise crash boolean parsing.
        """
        ...
        if isinstance(v, str):
            ...
            if s in {"false", "0", "no", "n", "off"}:
                return False
            if s in {"debug", "info", "warn", "warning", "error", "critical"}:
                return False
        return False
