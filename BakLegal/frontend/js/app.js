/* ── Config ───────────────────────────────────────────────────────────────── */
const API = "http://localhost:8000/similarity/api";
...
async function loadCategories() {
  try {
    ...
    setBackendBanner("panel-similarity",
      "⚠️ <strong>Similar Case Finder backend is offline.</strong> Run: <code>cd backend-M &amp;&amp; python app.py</code>",
      true);
    ...
    selCategory.innerHTML = '<option value="">⚠️ Backend offline</option>';
  }
}
...
selSubcategory.addEventListener("change", async () => {
  ...
  try {
    ...
    resultsList.innerHTML = `<div class="error-msg">⚠️ Could not load cases. Is the backend running?</div>`;
    resultsPanel.style.display = "block";
...
async function findSimilarToFile(filename, category, subcategory) {
  ...
  try {
    ...
    if (data.error) {
      html += `<div class="error-msg">⚠️ ${data.error}</div>`;
  ...
  Object.entries(data).sort().forEach(([cat, subcats]) => {
    subcats.forEach(({ subcategory, count }) => {
      ...
      card.innerHTML = `
        ...
        <button class="btn-view-cases" data-cat="${cat}" data-sub="${subcategory}" title="View Cases">📋</button>`;
  ...
  btnAddCase.addEventListener("click", async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        btnAddCase.textContent = "⚠ Duplicate";
        btnAddCase.style.background = "#e65100";
...
async function checkCmpBackend() {
  ...
  if (!ok) {
    setBackendBanner("panel-compliance",
      "⚠️ <strong>Compliance backend is offline.</strong> Run: <code>python server.py</code>",
      true);
  } else {
...
async function checkExtBackend() {
  ...
  if (!ok) {
    setBackendBanner("panel-extractor",
      "⚠️ <strong>Case Extractor backend is offline.</strong> Run: <code>python server.py</code>",
      true);
  } else {
...
const ARG_API = "http://localhost:8000/argument/api/v1";
...
async function checkArgBackend() {
  ...
  if (!ok) {
    setBackendBanner("panel-argument",
      "⚠️ <strong>Argument Scorer backend is offline.</strong> Run: <code>python server.py</code>",
      true);
  } else {
      ...
      return `
        <div class="arg-att-item">
          <div class="arg-att-header">
            <span class="arg-att-icon">⚠️</span>
...
document.getElementById("btn-arg-analyze").addEventListener("click", async () => {
  ...
  if (!text) {
    argStatus("Please enter argument text.", "error");
    return;
  }
  if (text.length < 50) {
    argStatus("Argument text must be at least 50 characters.", "error");
    return;
  }
...
const EXT_API = "http://localhost:8000/extractor/api/v1";
...
document.getElementById("btn-ext-upload")?.addEventListener("click", async () => {
  ...
  try {
    ...
    extStatus(statusEl,
      ...
      `📋 Status: <strong>${procData.status || "processing"}</strong>`,
      "success");
...
async function extViewDocument(docId) {
  ...
  try {
    ...
    if (extracted >= totalFields) {
      ...
      statusLabel.innerHTML = `⚠️ <strong>${extracted}/${totalFields} fields extracted</strong>`;
    }
    ...
    document.getElementById("eot-metadata").innerHTML = `
      ...
      ${meta.judges?.length ? `
      <h3 class="ext-section-title" style="margin-top:1.25rem">👨‍⚖️ Judges</h3>
    ...
    document.getElementById("eot-insights").innerHTML = hasInsights ? `
      ...
      <h3 class="ext-section-title">⚖️ Key Legal Issues</h3>
...
const CMP_API = "http://localhost:8000/compliance";
...
document.getElementById("btn-cmp-check")?.addEventListener("click", async () => {
  ...
  if (!pdfFile && !text) {
    cmpStatus(statusEl, "⚠️ Please upload a PDF or paste document text.", "warn");
    return;
  }
  ...
  try {
    ...
    if (data.valid === false) {
      const typeLabels = {
        court_judgment: "Court Judgment / Case Law",
        legislation: "Legislative Document",
        academic_paper: "Academic Paper",
        news_article: "News Article",
        unknown: "Unrecognized Document",
      };
      const detected = typeLabels[data.detected_type] || data.detected_type || "Unknown";
      cmpStatus(statusEl,
        `<div style="display:flex;align-items:flex-start;gap:.75rem">`
        + `<span style="font-size:1.5rem;line-height:1">📄</span>`
        + `<div>`
        + `<strong style="font-size:1rem">This document does not appear to be a contract or agreement</strong><br>`
        + `<span style="font-size:.88rem;opacity:.85">Detected as: <strong>${detected}</strong></span><br><br>`
        + `<span style="font-size:.88rem">${data.reason}</span>`
        + `</div></div>`,
        "error");
      cmpHideSpinner();
      return;
...
function cmpRenderResultPage(data) {
  ...
  // Mandatory + Acts row
  document.getElementById("cmp-action-row").innerHTML = `
    <div class="cmp-action-card" onclick="document.getElementById('cmp-mandatory-section')?.scrollIntoView({behavior:'smooth'})">
      <div class="cmp-action-icon">📋</div>
    ...
    <div class="cmp-action-card" onclick="cmpGoToActsLibrary()">
      <div class="cmp-action-icon">📜</div>
      <div class="cmp-action-info">
  ...
  // Store clauses for filtering
  window._cmpClauses = clauses;
  window._cmpData = data;
  cmpRenderClauses(clauses);
...
function cmpRenderMandatory(present, missing) {
  ...
  document.getElementById("cmp-present-grid").innerHTML = present.map(c => `
    <div class="cmp-mand-card cmp-mand-present">
      <div class="cmp-mand-card-icon">✅</div>
  ...
  `).join("") || '<p style="color:rgba(255,255,255,.7);font-size:.88rem;padding:.5rem 0">No present mandatory clauses detected.</p>';
  ...
  document.getElementById("cmp-missing-grid").innerHTML = missing.map(c => `
    <div class="cmp-mand-card cmp-mand-missing">
      <div class="cmp-mand-card-icon">⚠️</div>
      <div class="cmp-mand-card-body">
        <div class="cmp-mand-card-title">${c.clause || c.id}</div>
        <div class="cmp-mand-card-desc">${c.legal_basis || c.rule || ''}</div>
      </div>
    </div>
  `).join("") || '<p style="color:rgba(255,255,255,.7);font-size:.88rem;padding:.5rem 0">No missing mandatory clauses — great!</p>';
}
      ...
      <div class="cmp-hist-item" data-id="${aid}" style="border:1px solid var(--border);border-radius:10px;padding:.9rem 1rem;margin-bottom:.7rem;transition:box-shadow .2s">
        <div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:.4rem">
          ...
          <div style="display:flex;align-items:center;gap:.6rem;flex-shrink:0">
            ...
            <button onclick="cmpViewHistory('${aid}')" style="padding:.35rem;font-size:1rem;border:1px solid var(--primary);color:var(--primary);background:transparent;border-radius:6px;cursor:pointer;line-height:1;width:32px;height:32px;display:inline-flex;align-items:center;justify-content:center" title="View full report">👁</button>
            <button onclick="cmpDeleteHistory('${aid}')" style="padding:.35rem;font-size:1rem;border:1px solid #b71c1c;color:#b71c1c;background:transparent;border-radius:6px;cursor:pointer;line-height:1;width:32px;height:32px;display:inline-flex;align-items:center;justify-content:center" title="Delete this analysis">🗑</button>
          </div>
        </div>
