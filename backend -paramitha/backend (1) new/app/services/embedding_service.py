...
def _find_local_snapshot() -> Optional[str]:
    ...
    try:
        ...
        model_cache = _os.path.join(cache_root, f"models--{org}--{name}")
        refs_main = _os.path.join(model_cache, "refs", "main")
        if _os.path.exists(refs_main):
            with open(refs_main) as fh:
                commit_hash = fh.read().strip()
            snapshot_dir = _os.path.join(model_cache, "snapshots", commit_hash)
            # Check that the actual weights are present (not just config files)
        ...
        with _model_lock:
            if _model is None:
                try:
                    ...
                    local_path = _find_local_snapshot()
                    if local_path:
