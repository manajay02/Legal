...
class OCRService:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info(f"✓ Tesseract v{version} available")
        ...
        try:
            ...
            # Test with a small dummy PDF conversion
            import tempfile
            import os
            dummy_pdf = os.path.join(tempfile.gettempdir(), "test_poppler.txt")
            with open(dummy_pdf, "w") as f:
                f.write("test")  # This will fail which is fine - we just want to test poppler
            try:
                convert_from_path(dummy_pdf, first_page=1, last_page=1)
            except PDFInfoNotInstalledError:
                raise Exception("Poppler not installed")
            except Exception:
                pass  # Other errors are fine, poppler is available
            self.poppler_available = True
            logger.info("✓ Poppler available for PDF-to-image conversion")
        except Exception as e:
            logger.warning("✗ Poppler not available - OCR will be limited")
            logger.warning("  Install poppler for full OCR support: https://github.com/oschwartz10612/poppler-windows/releases")
        
        # Summary
        if self.tesseract_available and self.poppler_available:
            logger.info("Hybrid OCR Service: Full functionality (pdfplumber + Tesseract OCR)")
        elif self.tesseract_available:
            logger.info("Hybrid OCR Service: Fast extraction only (poppler missing)")
        else:
            logger.warning("OCR Service: Limited functionality (pdfplumber only)")
        logger.info("Strategy: Fast (pdfplumber) + OCR Fallback (when available)")
    ...
    def extract_page_with_ocr(self, pdf_path: str, page_num: int) -> str:
        ...
        if not self.poppler_available:
            logger.warning(f"  Page {page_num}: Poppler not available, skipping OCR")
            return ""
        ...
        try:
            ...
            try:
                ...
                error_msg = str(pdf2img_error).lower()
                if "poppler" in error_msg or "pdftoppm" in error_msg:
                    logger.warning(f"  Page {page_num}: Poppler not installed - skipping OCR (install poppler for full OCR support)")
                else:
                    logger.error(f"  Page {page_num}: PDF to image conversion failed - {str(pdf2img_error)}")
                return ""
